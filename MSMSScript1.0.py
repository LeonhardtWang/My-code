...
if __name__ == "__main__":
    while True:
        ...
        # 处理设备发送填写模板
        excel_name = '1246441196.设备发送填写模板.xlsx'
        ...
        # 处理设备下架填写模板
        excel_name = '1954872307.设备下架填写模板.xlsx'
        df = pd.read_excel(excel_name, sheet_name='Sheet2')
        exc_col = {}
        for i, c in enumerate(df.columns):
            exc_col[c] = convertToTitle(i+1)
        ...
        # 处理设备上架填写模板
        excel_name = '588199434.设备上架填写模板.xlsx'
        df = pd.read_excel(excel_name, sheet_name='Sheet2')
        exc_col = {}
        for i, c in enumerate(df.columns):
            exc_col[c] = convertToTitle(i+1)
        ...
        # 处理设备搬迁填写模板
        excel_name = '125440408.设备搬迁填写模板.xlsx'
        df = pd.read_excel(excel_name, sheet_name='Sheet2')
        exc_col = {}
        for i, c in enumerate(df.columns):
            exc_col[c] = convertToTitle(i+1)
        ...
        # 处理设备信息更改表
        excel_name = '2024737505.设备信息更改模板.xlsx'
        df = pd.read_excel(excel_name, sheet_name='Sheet3')
        exc_col = {}
        for i, c in enumerate(df.columns):
            exc_col[c] = convertToTitle(i+1)
        ...
        for i in range(60, -1, -1):
            print("脚本运行倒计时：还剩{}秒".format(str(i).rjust(2)), end='\r')
            time.sleep(1)
        print('\n')
